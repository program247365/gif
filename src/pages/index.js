  ...
  return (
    <article style={{ color: "#4D4D4F" }} className="bg-light-yellow">
      <Helmet>
        ...
        />
        <script
          src="https://unpkg.com/lazysizes@4.0.1/lazysizes.min.js"
          async=""
        />
      </Helmet>
      <h2 className="w-100 center avenir tc-l ph3 ph4-ns pt4 center">
        🎉 Lulz Gif Party! 🎉
      </h2>
      <div className="cf pa2">
        {data.allFile.edges.reverse().map(({ node }, index) => (
          <div className="fl w-50 w-25-m w-20-l pa2" key={index}>
            <a
              href={__PATH_PREFIX__ + node.relativePath}
            ...
            >
              {/*
                        ...
                        */}
              <img
                data-src={__PATH_PREFIX__ + node.relativePath}
                alt={node.name + "." + node.extension}
                className="lazyload w-100 db outline black-10 grow"
              />
              <dl className="mt2 f6 lh-copy">
                <dd
                  className="ml0 black truncate w-100"
                  data-clipboard-target={index + "copyme"}
                >
